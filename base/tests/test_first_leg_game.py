...
class FirstLegGameTest(TestCase):
    ...
    def test_first_leg_game_returns_true(self):
        ...
        league = League.objects.create(name='League 1', district=district, bhv_id=1)
        ...
        first_leg_game = Game.objects.create(number=1, league=league, opening_whistle=earlier_opening_whistle,
                                             ...
                                             report_number=1)
        Game.objects.create(number=2, league=league, opening_whistle=datetime.now(),
                            home_team=team_2, guest_team=team_1, home_goals=0, guest_goals=0,
                            report_number=2)
    ...
    def test_second_leg_game_returns_false(self):
        district = District.objects.create(bhv_id=1)
        league = League.objects.create(name='League 1', abbreviation='L1', district=district, bhv_id=1)
        team_1 = Team.objects.create(name='Team 1', short_name='T1', league=league, bhv_id=1)
        team_2 = Team.objects.create(name='Team 2', short_name='T2', league=league, bhv_id=2)
        ...
        second_leg_game = Game.objects.create(number=2, league=league, opening_whistle=later_opening_whistle,
                                              home_team=team_1, guest_team=team_2, home_goals=0, guest_goals=0,
                                              report_number=2)
...
def create_test_game(number, league, home_team, guest_team, should_be_first_leg) -> (Game, bool):
    ...
    game = Game.objects.create(number=number, league=league, opening_whistle=opening_whistle, home_team=home_team,
                               guest_team=guest_team, home_goals=0, guest_goals=0, report_number=number)
