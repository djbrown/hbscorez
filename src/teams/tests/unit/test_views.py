...
class TestDetails(ViewTestCase):
    def test(self):
        ...
        response = self.get_url("teams:detail", pk=team.pk)
        self.assertContains(response, team.name)

    def test_not_found(self):
        response = self.get_url("teams:detail", pk=1)
...
class TestGames(ViewTestCase):
    def test(self):
        ...
        response = self.get_url("teams:games", pk=team.pk)
    ...
    def test_not_found(self):
        response = self.get_url("teams:games", pk=1)
        self.assertEqual(response.status_code, 404)


class TestPlayers(ViewTestCase):
    def test(self):
        district = District.objects.create(name="Test District 10", bhv_id=10)
        season = Season.objects.create(start_year=2025)
        lg = League.objects.create(name="Test L 25", abbreviation="L25", bhv_id=25, district=district, season=season)
        team = Team.objects.create(name="Test Team 251", short_name="T251", bhv_id=251, league=lg)
        player1 = Player.objects.create(name="Test Player 2511", team=team)
        player2 = Player.objects.create(name="Test Player 2512", team=team)

        response = self.get_url("teams:players", pk=team.pk)
        self.assertContains(response, team.name)
    ...
    def test_not_found(self):
        response = self.get_url("teams:players", pk=1)
        self.assertEqual(response.status_code, 404)
...
class TestScorers(ViewTestCase):
    def test(self):
        ...
        response = self.get_url("teams:scorers", pk=team.pk)
        self.assertContains(response, team.name)
    ...
    def test_not_found(self):
        response = self.get_url("teams:scorers", pk=1)
        self.assertEqual(response.status_code, 404)
...
class TestOffenders(ViewTestCase):
    def test(self):
        ...
        response = self.get_url("teams:offenders", pk=team.pk)
        self.assertContains(response, team.name)
    ...
    def test_not_found(self):
        response = self.get_url("teams:offenders", pk=1)
        self.assertEqual(response.status_code, 404)


class TestCalendar(ViewTestCase):
    def test(self):
        district = District.objects.create(name="Test District 10", bhv_id=10)
        season = Season.objects.create(start_year=2025)
        lg = League.objects.create(name="Test L 25", abbreviation="L25", bhv_id=25, district=district, season=season)
        team = Team.objects.create(name="Test Team 251", short_name="T251", bhv_id=251, league=lg)
        hall = SportsHall.objects.create(number=3, name="Test Sports Hall 3", address="Test Adr 3", bhv_id=30)
        op = timezone.make_aware(datetime.now())
        Game.objects.create(
            number=2501, league=lg, opening_whistle=op, home_team=team, guest_team=team, sports_hall=hall
        )

        response = self.get_url("teams:calendar", pk=team.pk)
        self.assertContains(response, lg.name)
        self.assertContains(response, team.name)
        self.assertContains(response, hall.address)
    ...
    def test_not_found(self):
        response = self.get_url("teams:calendar", pk=1)
        self.assertEqual(response.status_code, 404)
